...
# resource "aws_iam_policy_attachment" "push_event_policy_attachment" {
#     name = "${local.app_name}-push-event-policy-attachment"
#     groups = [aws_iam_group.editor_group.name]
...
#     assume_role_policy = templatefile("policies/push_event_policy.tpl", {queue_arn = aws_sqs_queue.event_queue.arn})
# }
...
resource "aws_iam_policy" "assume_push_event_role_policy" {
  name   = "${local.app_name}-assume-push-event-role-policy"
  policy = templatefile("policies/assume_role_policy.tpl", { role_arn = aws_iam_policy.push_event_policy.arn })
}
...
#     groups = [aws_iam_group.editor_group.name]
#     policy_arn = aws_iam_policy.assume_push_event_role_policy.arn
# }
